...
export default function AppPage() {
  ...
  function downloadExport() {
    const payload = stringifyAppDataExport(state, new Date(), process.env.NEXT_PUBLIC_APP_VERSION);
    ...
    <div className="stack today-overview">
      <section className="action-grid">
        <button className="action-card hydration-action" onClick={() => props.setTab("hydration")}>
          ...
          <small>Chạm để xem chi tiết</small>
        </button>
        ...
        <button className="action-card workout-action" onClick={() => props.setTab("workout")}>
          ...
          <small>Chạm để vào buổi tập</small>
        </button>
      </section>
      ...
      <section className="hydration-block">
        ...
        <div className="hydration-history">
          {props.hydrationLogs.length ? (
            ...
            <EmptyState title="Chưa có log nước" text="Dùng một nút log nhanh phía trên để bắt đầu theo dõi hôm nay." />
          )}
        </div>
      </section>
            ...
            {props.state.workoutExercises.map((exercise, index) => (
              <div key={exercise.id} className={index === props.state.activeExerciseIndex ? "active" : ""}>
                ...
                <div className="exercise-edit-grid">
                  ...
                  <div className="wide-field previous-session-readout">
                    <span>Lần trước</span>
                    <strong>{exercise.lastSession}</strong>
                  </div>
                </div>
              </div>
            ))}
  ...
  return (
    <div className="stack workout-focus live-workout">
      ...
      <section className="control-card card">
        ...
        <div className="quick-adjust-grid" aria-label="Chỉnh nhanh set hiện tại">
          <button onClick={() => props.setSetWeight(Math.max(0, props.setWeight - 1))}>-1kg</button>
          <button onClick={() => props.setSetWeight(props.setWeight + 2.5)}>+2.5kg</button>
          <button onClick={() => props.setSetReps(Math.max(0, props.setReps - 1))}>-1 rep</button>
          <button onClick={() => props.setSetReps(props.setReps + 1)}>+1 rep</button>
        </div>
      </section>
      ...
      <section className="card">
        ...
        <div className="timeline compact">
          {props.currentSessionSets.length ? (
            ...
            <EmptyState title="Chưa ghi set" text="Hoàn thành set hiện tại để tạo lịch sử buổi tập." />
          )}
        </div>
      </section>
...
function ProgressView(props: {
  ...
  const coachPoints = [
    ...
    ...props.coachRecommendation.dataBasis,
    props.coachRecommendation.guardrail
  ].flatMap((text) => text.split(/(?<=[.!?])\s+/).filter(Boolean)).slice(0, 4);
  const hydrationBars = Array.from({ length: 7 }, (_, index) => {
    const hitDay = index < props.progress.hydration7.hitDays;
    return {
      label: `D${index + 1}`,
      height: hitDay ? 100 : Math.max(12, Math.min(100, (props.progress.hydration7.averageMl / Math.max(1, props.target)) * 100)),
      hitDay
    };
  });
    ...
    <div className="stack progress-screen">
      ...
      <section className="card">
        ...
        <div className="timeline compact">
          ...
          {!props.sharedPosts.length && <EmptyState title="Chưa có bài chia sẻ" text="Chia sẻ huy hiệu hoặc tóm tắt buổi tập khi bạn muốn công khai tiến độ." />}
        </div>
      </section>
      ...
      <section className="card">
        ...
        <div className="bar-chart labeled-chart" aria-label="Biểu đồ tiến độ">
          {hydrationBars.map((bar) => (
            <span key={bar.label} className={bar.hitDay ? "hit" : ""} style={{ height: `${bar.height}%` }}>
              <em>{bar.label}</em>
            </span>
          ))}
        </div>
