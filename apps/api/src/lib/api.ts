...
const serverState = currentApiState;
...
export async function syncBatch(input: { items?: SyncBatchItem[]; idempotencyKey?: string }): Promise<ApiResult<{ results: SyncBatchResult[] }>> {
  ...
  for (const [index, item] of input.items.entries()) {
    ...
    const cached = await runtime.repository.getSyncResult(runtime.user, key);
    if (cached && !isConfirmingConflict(item.payload)) {
      results.push(cached);
      continue;
    }
...
export async function notificationStatus(localProfileId = serverState().profile.email || "local-profile", env: NodeJS.ProcessEnv = process.env) {
  const runtime = currentApiRuntime();
  const subscriptions = await runtime.repository.listNotificationSubscriptions(runtime.user, { localProfileId });
  return ok({
    configured: isWebPushConfigured(env),
    fallbackMode: isWebPushConfigured(env) ? "web-push" : "in-app",
...
export async function subscribeNotifications(input: {
  endpoint?: string;
  p256dh?: string;
  auth?: string;
  ...
  const subscription = {
    ...
    localProfileId: input.localProfileId ?? runtime.user.email ?? serverState().profile.email ?? "local-profile",
    platform: input.platform,
    createdAt: new Date().toISOString()
  };
  return ok(await runtime.repository.upsertNotificationSubscription(runtime.user, subscription));
}

export async function unsubscribeNotifications(endpoint: string) {
  if (!endpoint) return fail("endpoint is required");
  const runtime = currentApiRuntime();
  const subscriptions = await runtime.repository.listNotificationSubscriptions(runtime.user, { endpoint });
  if (!subscriptions.length) return fail("subscription not found");
  return ok(await runtime.repository.revokeNotificationSubscription(runtime.user, endpoint));
}
...
async function sendToSubscriptions(payload: PushPayload, filter: { endpoint?: string; localProfileId?: string } = {}) {
  ...
  const failed: string[] = [];
  const runtime = currentApiRuntime();
  const targets = await runtime.repository.listNotificationSubscriptions(runtime.user, filter);
...
export function getAchievementsAndLeaderboard() {
  ...
  return ok({
    achievements,
    leaderboardVisible: serverState().profile.leaderboardPublic,
    leaderboard: [
