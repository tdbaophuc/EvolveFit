...
describe("routine import parser", () => {
  it("parses the public sample CSV without errors and includes session/day", () => {
    const rootPath = join(process.cwd(), "apps/web/public/samples/evolvefit-routine-template.csv");
    const workspacePath = join(process.cwd(), "../../apps/web/public/samples/evolvefit-routine-template.csv");
    const sample = readFileSync(existsSync(rootPath) ? rootPath : workspacePath, "utf-8");
